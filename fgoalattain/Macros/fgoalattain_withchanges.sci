...
function [x,fval,attainfactor,exitflag,output,lambda] = fgoalattain(varargin)
 ...
 // function handle defining new nonlinfun function
   ...
   if(flag2==1) then
       ...
       g_attainoptions(pos2)="ON"
       updated_nonlinfun = new_nonlinfun_with_der
    else
       g_attainoptions(pos1)="ON"
       g_attainoptions(pos2)="OFF"
       updated_nonlinfun = new_nonlinfun_witout_der
    end
    
    //flag1=1
    //dnc=matrix(dnc,size(dnc,"c"),size(dnc,"r"))
    //dnceq=matrix(dnceq,size(dnceq,"c"),size(dnceq,"r"))
    ...
    //der_newfunc=matrix(der_newfunc,size(der_newfunc,"c"),size(der_newfunc,"r"))
    fder=der_newfunc
    endfunction
    //disp(startpoint)
